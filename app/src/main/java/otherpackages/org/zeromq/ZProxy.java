...
import otherpackages.org.zeromq.ZActor.Actor;
import otherpackages.org.zeromq.ZAgent.SelectorCreator;
import otherpackages.org.zeromq.ZMQ.Socket;
import otherpackages.org.zeromq.ZStar.Exit;

import otherpackages.zmq.Msg;
    ...
    private static final class ZmqPump implements Pump
    {
                ...
                Plug dplug, Socket destination)
        {
            ...
            while (true) {
                ...
                long more = src.getSocketOpt(otherpackages.zmq.ZMQ.ZMQ_RCVMORE);
                ...
                if (cpt != null) {
                    ...
                    rc = cpt.send(ctrl, more > 0 ? otherpackages.zmq.ZMQ.ZMQ_SNDMORE : 0);
                    if (!rc) {
                        // not successful, but we can still try to send it to the destination
                    }
                }
                ...
                // we send the packet
                rc = dst.send(msg, more > 0 ? otherpackages.zmq.ZMQ.ZMQ_SNDMORE : 0);
